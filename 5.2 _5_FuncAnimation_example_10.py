...
# f is the maximum value for x=m; as s gets smaller it also, gives larger max value
max_f = f(m, m, s_min)
...
def init():
    lines[0].set_data([], [])  # plot with nothing
    return lines
...
def frame(args):
    ...
    return lines
anim = animation.FuncAnimation(fig, frame, all_args, interval=150, init_func=init, blit=True)
anim.save('movie1.mp4', fps=5)  # movie formatted into a mp4 file